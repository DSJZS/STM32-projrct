...
/**
  * 函    数：OLED显示图像
  * 参    数：X 指定图像左上角的横坐标，范围：-127~127
  * 参    数：Y 指定图像左上角的纵坐标，范围：-63~63
  * 参    数：Width 指定图像的宽度，范围：0~128
  * 参    数：Height 指定图像的高度，范围：0~64
  * 参    数：Image 指定要显示的图像
  * 返 回 值：无
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  * 补充说明：此函数是在江协科技原函数的基础上经过了本人的略微修改
  */
...
void OLED_CopyDisplayBuf(uint8_t OLED_TmpBuf[8][128])
{
	uint8_t i = 0;
	for(i=0;i<8;++i)
	{
		...
		for(j=0;j<128;++j)
		{
			OLED_TmpBuf[i][j] = OLED_DisplayBuf[i][j];
		}
	}
}
